    ...
    <main>
        <section class="margin">
            ...
            <ul>
                ...
                <li>
                    <a href="#past_trips" class="yellow">My Past Trips</a>
                </li>
            </ul>
        </section>
        ...
        <section id="bookingen" class="margin">
            <h2>My Bookings</h2>
            <p>Here you can view and manage your upcoming bookings.</p>
            <?php
            $stmt = $pdo->prepare("SELECT * FROM booking WHERE user_id = :user_id and trip_end >= CURDATE() ORDER BY trip_start ASC");
        ...
        <section id="past_trips" class="margin">
            <h2>My Past Trips</h2>
            <p>Here you can view and manage your past bookings.</p>
            <?php
            $stmt = $pdo->prepare("SELECT * FROM booking WHERE user_id = :user_id and trip_end < CURDATE() ORDER BY trip_start DESC");
            $stmt->bindParam(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
            $stmt->execute();
            $bookings = $stmt->fetchAll(PDO::FETCH_ASSOC);
